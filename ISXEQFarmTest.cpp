...
#include "ISXEQFarmTest.h"

// The mandatory pre-setup function.  Our name is "ISXEQFarmTest", and the class is ISXEQFarmTest.
...
// our instanced class.
ISXPreSetup("ISXEQFarmTest", ISXEQFarmTest);
...
unsigned int ISXEQFarmTestXML = 0;
...
bool ISXEQFarmTest::Initialize(ISInterface *p_ISInterface)
{
	...
	WriteChatf("ISXEQFarmTest Loaded");
	return true;
}

// shutdown sequence
void ISXEQFarmTest::Shutdown()
{
	// save settings, if you changed them and want to save them now.  You should normally save
	// changes immediately.
	//pISInterface->ExportSet(ISXEQFarmTestXML,XMLFileName);

	pISInterface->UnloadSet(ISXEQFarmTestXML);
...
void ISXEQFarmTest::ConnectServices()
{
...
void ISXEQFarmTest::RegisterCommands()
{
	// add any commands
...
void ISXEQFarmTest::RegisterAliases()
{
	// add any aliases
}

void ISXEQFarmTest::RegisterDataTypes()
{
	// add any datatypes
	// pMyType = new MyType;
...
void ISXEQFarmTest::UnRegisterCommands()
{
	// remove commands
	//    pISInterface->RemoveCommand("MyCommand");
}
void ISXEQFarmTest::UnRegisterAliases()
{
	// remove aliases
}
void ISXEQFarmTest::UnRegisterDataTypes()
{
	// remove data types
	//if (pMyType)
	//{
...
void ISXEQFarmTest::UnRegisterTopLevelObjects()
{
	// remove Top-Level Objects
	//    pISInterface->RemoveTopLevelObject("MapSpawn");
}
void ISXEQFarmTest::UnRegisterServices()
{
	// shutdown our own services
	//    if (hISXEQFarmTestService)
	//        pISInterface->ShutdownService(this,hISXEQFarmTestService);
}

void ISXEQFarmTest::LoadSettings()
{
	bool bChanged = false;

	// open the settings file, and store the ID number in ISXEQFarmTestXML
	ISXEQFarmTestXML = pISInterface->OpenSettings(XMLFileName);
	...
	Any set can be referred to by its ID.  The base set is stored in ISXEQFarmTestXML.  Note that the settings
	...
	*/
	...
	if (unsigned int GeneralGUID = pISInterface->FindSet(ISXEQFarmTestXML, "General"))
	{
